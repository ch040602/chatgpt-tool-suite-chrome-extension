...
긴 대화, 브랜치, 다음 질문 대기, Office 수식 복사를 한곳에서 다루는 ChatGPT용 Chrome MV3 도구 모음입니다.

확장은 다음 기능을 하나의 도구 모음처럼 제공합니다.

- 긴 ChatGPT 대화의 브라우저 로딩/RAM 압박 완화
- 브랜치 요약 추적과 현재 세션 바로가기
- 편집 가능한 다음 질문 queue
- Word/PowerPoint용 안전한 수식 복사
- unpacked 설치용 가벼운 업데이트 도우미
...
v1.5.3은 기존 긴 대화 loader에서 ChatGPT 도구 모음 성격으로 확장한 버전입니다. v1.5.2의 긴 대화/Office 수식 보정을 유지하면서 브랜치 추적과 다음 질문 queue를 추가합니다.
...
- 현재 ChatGPT 대화 경로와 연결된 branch variant를 작은 브랜치 패널로 표시합니다. 펼친 화면은 모든 prompt를 나열하지 않고, 분기 전 마지막 prompt와 각 branch 시작 첫 prompt만 요약합니다.
- 기본 `Tab` 또는 popup에서 실제 키를 한 번 눌러 등록한 단축키로 prompt들을 queue에 넣고, 대기 목록 패널에서 하나씩 확인/수정한 뒤 현재 응답과 요청 제한 문구가 사라지면 순서대로 전송합니다.
...
**브랜치 경로 표시**가 켜져 있으면 ChatGPT 탭에 현재 대화 경로와 탭에서 이전에 관측된 연결 branch variant를 함께 표시합니다. **브랜치 패널 단축키**로 열거나 접을 수 있으며 기본값은 `Alt+B`입니다. 접힌 미니 오버레이는 그대로 클릭할 수 있고 prompt 텍스트 없이 단순 graph만 보여줍니다. 펼친 화면은 분기 요약만 표시합니다: 분기 전 마지막 user prompt와 각 branch 시작 첫 user prompt입니다. branch 요약을 클릭하면 현재 세션에 보이는 해당 대화 지점으로 바로 이동합니다.

**다음 질문 대기 전송**이 켜져 있으면 ChatGPT가 답변 중일 때 prompt를 입력하고 지정 queue 단축키를 누릅니다. 기본값은 `Tab`입니다. queue된 prompt는 편집 가능한 대기 목록 패널에 쌓입니다. **대기 목록 단축키** 기본값은 `Alt+Q`이고, 떠 있는 **Queue** 버튼이나 접힌 queue 미니 오버레이를 클릭해서도 열 수 있습니다. 단축키를 바꾸려면 popup의 단축키 입력칸에 focus한 뒤 원하는 키 조합을 한 번 누릅니다. 확장은 live response 표시가 사라질 때까지 기다린 뒤 `Too many requests`, `rate limit` 같은 요청 제한 문구가 보이는지 확인합니다. 해당 문구가 있으면 전송하지 않고 대기하며, queued prompt를 하나씩 순서대로 보냅니다.
...
빌드 과정은 필요 없습니다. 저장소 root 자체가 Chrome unpacked extension
폴더입니다.
...
release ZIP이 아니라 GitHub 저장소에서 바로 설치할 때는 다음 순서로 진행합니다.

1. 저장소를 엽니다:
   `https://github.com/ch040602/chatgpt-tool-suite-chrome-extension`
2. **Code** -> **Download ZIP**을 누릅니다.
3. ZIP 압축을 풉니다.
4. `chrome://extensions`에서 **개발자 모드**를 켜고 **압축해제된 확장 프로그램을 로드합니다**를 누릅니다.
5. `manifest.json`이 바로 들어 있는 폴더를 선택합니다.
6. 이미 열려 있던 ChatGPT 탭을 새로고침합니다.

나중에 unpacked 설치를 업데이트하려면 새 ZIP을 다운로드해 같은 폴더에
압축 해제한 뒤, `chrome://extensions`에서 해당 확장의 reload 버튼을
누릅니다.

### 기본 프로필

확장은 제가 현재 사용하는 설정을 기본값으로 포함합니다. 새로 설치한 뒤
별도 설정 없이 바로 사용할 수 있습니다.

- popup 기본 언어: 한국어
- 긴 대화 trim 및 네트워크 안전 모드: 켜짐
- 처음 표시할 최근 턴: `2`
- 더 보기 배치: `2`
- API 사전 보관 배치: `0`
- 응답 micro-cache 수: `1`, 캐시 항목 상한: `256 KB`
- 대화 중 자동 정리 및 불러온 과거 메시지 자동 접기: 켜짐
- 브랜치 경로 패널: 켜짐, 단축키 `Alt+B`
- 다음 질문 대기 전송: 켜짐, 단축키 `Tab`
- 대기 목록 열기/접기 단축키: `Alt+Q`
- Office 수식 복사 및 PNG fallback: 켜짐
- 상태 배지와 debug logging: 꺼짐

기존에 설정을 바꿨다면 popup에서 **빠른 초기 로딩 프리셋**을 눌러
성능 중심 프로필로 되돌린 뒤 ChatGPT 탭을 새로고침합니다.
...
fork를 공개할 경우 저작권자, 저장소 이름, release asset 이름, README 링크를 본인 기준에 맞게 수정해야 합니다. 기준 저장소 이름은 `chatgpt-tool-suite-chrome-extension`입니다.
...
5. 확장 폴더 root 기준으로 ZIP을 생성합니다. 권장 asset 이름은 `chatgpt-tool-suite-chrome-extension-v1.5.3.zip`입니다.
